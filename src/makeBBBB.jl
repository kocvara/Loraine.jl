...
function makeBBBBsi(ilmi,Ailmi,AAilmi,myA,Wilmi,n,to,qA,sigmaA)
    BBBB = zeros(Float64x4, n, n)
    tmp1 = Matrix{Float64x4}(undef,size(Wilmi, 2), size(Ailmi[1], 1))
    # tmp = Matrix{Float64}(undef,size(Wilmi, 1), size(Wilmi, 1))
    tmp2 = Matrix{Float64x4}(undef,size(AAilmi, 1), 1)
    ...
    @inbounds for ii = 1:n
        ...
        tmp  = zeros(Float64x4,size(Wilmi, 2), size(Ailmi[1], 1))
        i = sigmaA[ii,ilmi]
        # if ii <= qA[1]
        if 1==1
             # @show "one"
